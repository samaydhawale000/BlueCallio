...
type Customer = { id: string; name: string | null; email: string; hasPaymentMethod: boolean; status: string; lastActiveAt: string | null; projectCount: number; minutesUsed: number };
...
export default function AdminCustomersPage() {
  ...
  if (loading) return <div className="text-[#6B6478] text-sm py-20 text-center">Loading customers…</div>;
  if (error) return <div className="py-20 text-center"><p className="text-red-600 text-sm">{error}</p></div>;
  return <div className="space-y-6"><header><h1 className="text-2xl font-bold text-[#170B2E]">Customers</h1><p className="text-[#6B6478] text-sm mt-1">{total} registered companies</p></header>
    <div className="rounded-xl border border-[#E7DFF5] overflow-hidden" style={{ background: '#FFFFFF' }}><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-[#E7DFF5] text-left">{['Company', 'Owner', 'Billing', 'Projects', 'Minutes Used', 'Last Active', 'Status', 'Actions'].map((label) => <th key={label} className="px-4 py-3 text-xs font-mono uppercase tracking-widest text-[#8A8298]">{label}</th>)}</tr></thead><tbody>{customers.map((c) => <tr key={c.id} className="border-b border-[#E7DFF5]/60"><td className="px-4 py-3 text-[#170B2E] font-medium">{c.name || 'Unnamed'}</td><td className="px-4 py-3 text-[#6B6478]">{c.email}</td><td className="px-4 py-3"><span className={`text-[11px] px-2 py-1 rounded ${c.hasPaymentMethod ? 'text-emerald-700 border border-emerald-500/30' : 'text-[#8A8298] border border-[#E7DFF5]'}`}>{c.hasPaymentMethod ? 'Card on file' : 'No card'}</span></td><td className="px-4 py-3 text-[#4B4560]">{c.projectCount}</td><td className="px-4 py-3 text-[#4B4560]">{c.minutesUsed}</td><td className="px-4 py-3 text-[#6B6478]">{c.lastActiveAt ? new Date(c.lastActiveAt).toLocaleDateString() : '—'}</td><td className="px-4 py-3 text-[#4B4560]">{c.status}</td><td className="px-4 py-3"><button onClick={() => update(c.id, 'status', { status: c.status === 'SUSPENDED' ? 'ACTIVE' : 'SUSPENDED' })} className="text-[11px] px-2.5 py-1 rounded border border-red-500/40 text-red-600">{c.status === 'ACTIVE' ? 'Suspend' : 'Resume'}</button></td></tr>)}{customers.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-[#8A8298]">No customers yet.</td></tr>}</tbody></table></div><div className="px-4 pb-4"><Pagination page={page} pageCount={pageCount} totalItems={total} pageSize={pageSize} onPageChange={setPage} /></div></div>
  </div>;
}
