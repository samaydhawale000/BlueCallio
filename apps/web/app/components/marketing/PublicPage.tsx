...
export function PublicPage({ eyebrow, title, intro, crumbs, children }: { eyebrow: string; title: string; intro: string; crumbs: Crumb[]; children: React.ReactNode }) {
  return <main className="min-h-screen bg-[#060B18] px-6 pb-24 pt-28 text-slate-300">
    <div className="mx-auto max-w-5xl">
      <Breadcrumbs items={crumbs} />
      <p className="mb-4 font-mono text-xs uppercase tracking-widest text-indigo-300">{eyebrow}</p>
...
export function ContentSection({ title, children }: { title: string; children: React.ReactNode }) {
  return <section><h2 className="text-2xl font-bold text-white">{title}</h2><div className="mt-4 max-w-4xl space-y-4 leading-7 text-slate-400">{children}</div></section>;
}
