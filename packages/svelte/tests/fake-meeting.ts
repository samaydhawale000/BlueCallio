 ...
 * Mirrors just the public surface `createPurpleCallio` depends on, with no
 * real WebRTC/socket activity, so the factory can be unit tested in plain
 * Node without a browser. Mirrors the sibling `@purplecallio/angular` and
 * `@purplecallio/react-native` packages' `tests/fake-meeting.ts`.
 */
